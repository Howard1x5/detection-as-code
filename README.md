...
1. **Detection-as-Code → [triage-localLLM](https://github.com/Howard1x5/triage-localLLM)** — alerts generated by these Sigma/YARA/Wazuh detections feed into triage-localLLM for automated first-pass classification (severity, MITRE mapping, confidence-scored escalation).
2. **triage-localLLM → [ARGUS](https://github.com/Howard1x5/argus)** — anything triage flags as high-severity or low-confidence hands off to ARGUS for full investigation against the underlying sample/evidence.
3. **[SEDT](https://github.com/Howard1x5/sedt)** — intended as the synthetic data source for validating these detections against realistic telemetry. Currently generates benign worker-simulation data only; extending it to include adversarial/attack behavior is the next step toward closing this loop end-to-end (see SEDT's own roadmap note).

This turns three independently-built projects into one coherent detection → triage → investigation pipeline, mirroring a real L1→L2 SOC escalation tier.
